...
class Calendar:
    ...
    def __init__(s, point=None, position=None):
        ...
        s._selection = None  # ����Ϊδѡ������

        s.G_Frame = ttk.Frame(s.master)

        s._cal = s.__get_calendar(locale, fwday)

        s.__setup_styles()  # �����Զ�����ʽ
        s.__place_widgets()  # pack/grid С����
        s.__config_calendar()  # ���������кͰ�װ���
        # ���û�������ȷ�İ󶨣���ѡ�����ڡ�
        ...
        s.master.geometry('%dx%d+%d+%d' % (width, height, x, y))  # ����λ�þ���
        s.master.after(300, s._main_judge)
        s.master.deiconify()
        s.master.focus_set()
        s.master.wait_window()  # ����Ӧ��ʹ��wait_window���𴰿ڣ����ʹ��mainloop,���ܻᵼ��������ܶ����
    ...
    def __place_widgets(s):
        # ��ͷ��ܼ���С����
        Input_judgment_num = s.master.register(s.Input_judgment)  # ��Ҫ��������װһ�£���Ҫ��
        ...
        tk.Label(hframe, text='��', justify='left').grid(in_=hframe, column=2, row=0, padx=(0, 5))
        ...
        tk.Label(hframe, text='��', justify='left').grid(in_=hframe, column=4, row=0)

        # ��������
        s._calendar = ttk.Treeview(gframe, show='', selectmode='none', height=7)
        s._calendar.pack(expand=1, fill='both', side='bottom', padx=5)

        ttk.Button(bframe, text="ȷ ��", width=6, command=lambda: s._exit(True)).grid(row=0, column=0, sticky='ns',
                                                                                    padx=20)
        ttk.Button(bframe, text="ȡ ��", width=6, command=s._exit).grid(row=0, column=1, sticky='ne', padx=20)
...
if __name__ == '__main__':
    ...
    width, height = root.winfo_reqwidth() + 50, 50  # ���ڴ�С
    x, y = (root.winfo_screenwidth() - width) / 2, (root.winfo_screenheight() - height) / 2
    root.geometry('%dx%d+%d+%d' % (width, height, x, y))  # ����λ�þ���
    ...
    # Calendar((x, y), 'ur').selection() ��ȡ���ڣ�x,yΪ������
    date_str_gain = lambda: [
        date_str.set(date)
        for date in [Calendar((x, y), 'ur').selection()]
        if date]
    tk.Button(root, text='����:', command=date_str_gain).place(x=0, y=0, relx=1 / 20, rely=1 / 6, relwidth=4 / 20,
                                                             relheigh=2 / 3)
    root.mainloop()
